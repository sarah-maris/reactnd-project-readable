    ...
    return (
      <div className="post-changes">
        <Modal
          ...
          contentLabel="Modal"
        >
          <div className="close-click" onClick={this.closeEditPostModal}>
            X
          </div>
          {postModalOpen && (
            <EditPostForm initialValues={post} onSubmit={this.editPost} />
          )}
        </Modal>
        <div
          className="delete-white icon"
          onClick={this.deletePost.bind(this, post.id)}
        />
        <div className="edit-white icon" onClick={this.openEditPostModal} />
      </div>
    );
  }
}
