...
class SinglePost extends Component {
  ...
  componentWillReceiveProps({ location }) {
    const redirectPath = location.pathname.substr(
      0,
  ...
  render() {
    ...
    if (!post.id) return <Redirect to={{ pathname: this.state.backPath }} />;
    else
      return (
        <div>
          {!post.id ? (
            ...
            <article className="single">
              ...
              {post.id && (
                <CommentList postComments={postComments} postId={post.id} />
              )}
            </article>
          )}
        </div>
